...
export async function runFranchiseeLeadsReport(dateString: string, recipients: string[], fromAddress?: string) {
  ...
  // Load Franchisee Users map/sets to identify franchisee accounts
...
<body style="margin: 0; padding: 0; background-color: #f4f7f8; -webkit-text-size-adjust: 100%;">
    ...
    <tr>
      <td align="center">
        <table align="center" width="600" border="0" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; border: 1px solid #e2e8f0; overflow: hidden; border-collapse: separate;">
          ...
          <tr>
            <td style="padding: 30px 25px; background-color: #ffffff;">
              <h2 style="margin: 0 0 10px; font-size: 20px; color: #095c7b; font-family: 'Inter', system-ui, -apple-system, sans-serif; font-weight: 700;">Daily Franchisee Generated Leads Report</h2>
              <p style="margin: 0 0 20px; font-size: 14px; color: #4a5568; line-height: 1.5; font-family: 'Inter', system-ui, -apple-system, sans-serif;">
                Daily summary of leads created yesterday (<strong>${dateString}</strong>) by users with the Franchisee role.
              </p>
  ...
  const result = await sendAutomatedEmail({
    to: toStr,
    subject: `Daily Franchisee Generated Leads Report - ${dateString}`,
    html: emailHtml,
    customFrom: fromAddress || 'ankith.ravindran@mailplus.com.au'
  });
 ...
 */
export const sendDailyFranchiseeLeadsReport = functions
  .region("australia-southeast1")
  .pubsub.schedule("0 * * * *")
  .timeZone("Australia/Sydney")
  .onRun(async (context) => {
    functions.logger.info("Executing scheduled sendDailyFranchiseeLeadsReport function...");
    ...
    try {
      ...
      functions.logger.error("Failed to load daily_franchisee_leads_report recipients list", err);
    }

    if (frequency === "disabled") {
      functions.logger.info("Daily franchisee leads report is disabled. Skipping execution.");
      return;
    }
    ...
    try {
      await runFranchiseeLeadsReport(dateString, recipients, fromAddress);
    } catch (err) {
      functions.logger.error("Error executing daily franchisee leads report:", err);
    }
  });
