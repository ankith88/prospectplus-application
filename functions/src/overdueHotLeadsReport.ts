...
export async function runOverdueHotLeadsReport(recipients: string[], fromAddress?: string): Promise<any> {
  ...
  for (const doc of allHotLeadsDocs) {
    ...
    if (lead.activities && Array.isArray(lead.activities) && lead.activities.length > 0) {
      ...
      if (actDates.length > 0) {
        actDates.sort((a: Date, b: Date) => b.getTime() - a.getTime());
        lastAction = actDates[0];
      }
    }

    // 5. Calculate business hours overdue (> 8 hours SLA)
    const bizHoursElapsed = calculateBusinessHoursSydney(lastAction, now);
    if (bizHoursElapsed <= 8) continue;
...
<body style="margin: 0; padding: 0; background-color: #f4f7f8; -webkit-text-size-adjust: 100%;">
  ...
  return await sendAutomatedEmail({
    to: recipients.join(', '),
...
export const dailyOverdueHotLeadsReport = functions
  ...
  .onRun(async () => {
    ...
    try {
      ...
      if (configDoc.exists) {
        ...
        await db.collection('settings').doc('daily_overdue_hot_leads_report').set({
          recipients,
          frequency,
          fromAddress
        });
      ...
      functions.logger.error('Failed to load daily_overdue_hot_leads_report settings', err);
    }

    if (frequency === 'disabled') {
      functions.logger.info('Daily overdue hot leads report is disabled. Skipping execution.');
      return;
    }
    ...
    try {
      await runOverdueHotLeadsReport(recipients, fromAddress);
      functions.logger.info('Successfully sent Daily Overdue Inbound Hot Leads Report');
    } catch (error) {
      functions.logger.error('Error executing Daily Overdue Inbound Hot Leads Report:', error);
    }
  });
