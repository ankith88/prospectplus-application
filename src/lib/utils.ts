...
export function isManualActivity(act: any): boolean {
  ...
  // 1. Check explicit automated / system flags
  if (
  ...
  if (['system', 'automation', 'campaign', 'nurture', 'api', 'webhook', 'netsuite', 'bot', 'cron'].includes(source)) {
    return false;
  }
  ...
  // 4. Check notes / content / title / event for automated patterns
  const noteText = (act.notes || act.content || act.event || act.title || '').toString().toLowerCase();
  
  // Automated status/bucket/stage transitions
  if (
  ...
  // Automated integrations / logs / syncs
  if (
