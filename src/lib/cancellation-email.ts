...
export async function sendCancellationNotificationEmail(data: CancellationNotificationData) {
  try {
    const {
      leadId,
      netsuiteId,
    ...
    const retentionCenterUrl = `${baseUrl}/customer-success/cancellations`;

    const subject = `Cancellation Request: ${companyName}${processedBy ? ` (Processed by: ${processedBy})` : ''}`;
...
<head>
  ...
  <title>Cancellation Request Notification</title>
...
<body style="margin: 0; padding: 0; width: 100% !important; background-color: #f4f7f8; font-family: 'Inter', system-ui, -apple-system, sans-serif;">
  <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: #f4f7f8; padding: 20px 0; width: 100%;">
          ...
          <tr>
            <td class="content-cell" style="padding: 45px 35px; color: #2d3748; font-size: 15px; line-height: 1.6; font-family: 'Inter', system-ui, -apple-system, sans-serif;">
              ...
              <div class="greeting" style="font-size: 20px; color: #095c7b; font-weight: 700; margin-bottom: 16px; letter-spacing: -0.5px; font-family: 'Inter', system-ui, -apple-system, sans-serif;">
                New Cancellation Request Created
              </div>
              
              <p style="margin: 0 0 16px; font-size: 15px; color: #2d3748; font-family: 'Inter', system-ui, -apple-system, sans-serif; line-height: 1.6;">
                A new cancellation request has been logged for <strong>${companyName}</strong>. All associated details are outlined below.
              </p>
              ...
              <table width="100%" border="0" cellpadding="0" cellspacing="0" style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; margin: 20px 0; padding: 16px;">
                <tr>
                  <td style="font-size: 14px; color: #4a5568; font-family: 'Inter', system-ui, -apple-system, sans-serif; line-height: 1.6;">
                    ...
                    ${netsuiteId ? `<p style="margin: 0 0 8px;"><strong style="color: #095c7b;">NetSuite / Lead ID:</strong> ${netsuiteId}</p>` : (leadId ? `<p style="margin: 0 0 8px;"><strong style="color: #095c7b;">Lead ID:</strong> ${leadId}</p>` : '')}
                    ${contactName ? `<p style="margin: 0 0 8px;"><strong style="color: #095c7b;">Contact Name:</strong> ${contactName}</p>` : ''}
                    ${contactEmail ? `<p style="margin: 0 0 8px;"><strong style="color: #095c7b;">Contact Email:</strong> <a href="mailto:${contactEmail}" style="color: #095c7b; text-decoration: underline;">${contactEmail}</a></p>` : ''}
                    ${contactPhone ? `<p style="margin: 0 0 8px;"><strong style="color: #095c7b;">Contact Phone:</strong> ${contactPhone}</p>` : ''}
                    <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 12px 0;" />
                    ${cancellationTheme ? `<p style="margin: 0 0 8px;"><strong style="color: #095c7b;">Cancellation Theme:</strong> ${cancellationTheme}</p>` : ''}
                    ${cancellationWhy ? `<p style="margin: 0 0 8px;"><strong style="color: #095c7b;">Category / Why:</strong> ${cancellationWhy}</p>` : ''}
                    ...
                    ${trueServiceCancellationDate ? `<p style="margin: 0 0 8px;"><strong style="color: #095c7b;">True Service Stop Date:</strong> ${trueServiceCancellationDate}</p>` : ''}
                    ${cancellationNotes ? `<p style="margin: 0 0 8px;"><strong style="color: #095c7b;">Cancellation Notes:</strong> ${cancellationNotes}</p>` : ''}
                    <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 12px 0;" />
                    <p style="margin: 0;"><strong style="color: #095c7b;">Processed / Submitted By:</strong> ${processedBy || 'System'}</p>
                  </td>
                </tr>
              </table>

              <!-- Button to Customer Retention Center -->
              <table border="0" cellpadding="0" cellspacing="0" style="margin: 28px 0 20px;">
                <tr>
                  <td align="center" style="border-radius: 6px; background-color: #095c7b;">
                    <a href="${retentionCenterUrl}" target="_blank" style="font-size: 15px; font-family: 'Inter', system-ui, -apple-system, sans-serif; color: #ffffff; text-decoration: none; border-radius: 6px; padding: 14px 28px; display: inline-block; font-weight: 600; background-color: #095c7b;">
                      Open Customer Retention Center &rarr;
                    </a>
                  </td>
                </tr>
              </table>
    ...
    console.error('[Cancellation Email Error] Failed to send cancellation notification email:', error);
