 ...
 */
export async function findLeadByIdOrInternalId(rawId: string): Promise<{ lead: Lead; leadId: string } | null> {
  ...
  for (const colName of collections) {
    try {
      ...
      if (docSnap.exists) {
        return { lead: { id: docSnap.id, ...docSnap.data() } as Lead, leadId: docSnap.id };
      }
    } catch (err) {
  ...
  for (const colName of collections) {
    for (const field of searchFields) {
      ...
      if (!snapString.empty) {
        ...
        return { lead: { id: doc.id, ...doc.data() } as Lead, leadId: doc.id };
      }
