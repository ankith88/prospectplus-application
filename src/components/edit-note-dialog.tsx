...
import { updateNoteActivity } from '@/services/firebase'
...
export function EditNoteDialog({ lead, note, onNoteUpdated, isOpen, onOpenChange }: EditNoteDialogProps) {
  ...
  // Set form value when note changes
  useEffect(() => {
    if (note) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (note.author !== user.displayName) {
        toast({
            variant: 'destructive',
            title: 'Error',
            description: 'You can only edit notes created by yourself.',
        });
        return;
    }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px]" onInteractOutside={(e) => {
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              Modify your note for {lead.companyName}.
            </DialogDescription>
          </DialogHeader>
