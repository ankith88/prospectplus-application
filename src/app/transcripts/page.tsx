...
export default function TranscriptsPage() {
  ...
  const hasAccess = userProfile?.role && ['admin', 'Marketing Admin', 'Marketing Manager', 'user'].includes(userProfile.role);
  ...
  const fetchTranscripts = async () => {
    ...
    try {
      ...
      if (userProfile?.role === 'admin' || userProfile?.role === 'Marketing Admin' || userProfile?.role === 'Marketing Manager') {
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            ...
            <p className="text-muted-foreground">Review call transcripts. {(userProfile?.role === 'admin' || userProfile?.role === 'Marketing Admin' || userProfile?.role === 'Marketing Manager') ? 'Showing all transcripts.' : 'Showing your transcripts.'}</p>
        </div>
