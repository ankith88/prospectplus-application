...
export async function POST(request: Request) {
  ...
  <body style="margin: 0; padding: 0; background-color: #f4f7f8; -webkit-text-size-adjust: 100%;">
    ...
    try {
      ...
      console.warn('Failed to load daily_sales_snapshot_report settings:', dbErr);
    }
    ...
    return NextResponse.json({ message: `Daily sales snapshot report manually triggered successfully for ${recipients.join(', ')}` });
  } catch (error: any) {
    console.error('Error sending test sales snapshot report:', error);
