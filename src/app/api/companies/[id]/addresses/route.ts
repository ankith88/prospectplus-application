...
export async function POST(
  ...
  try {
    ...
    if (type === 'postal') {
      ...
      if (addressId) {
        const subDocRef = companyRef.collection('addresses').doc(addressId);
        ...
        addressData.createdAt = FieldValue.serverTimestamp();
        const newSubDocRef = await companyRef.collection('addresses').add(addressData);
    ...
    console.error('Error handling company address API:', error);
